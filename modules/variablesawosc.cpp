...
using namespace daisysp;
  ...
  float VariableSawOscillator::Process(){
      ...
      if (!high_ && phase_ >= pw_) {
        const float triangle_step = (slope_up + slope_down) * frequency_ * triangle_amount;
        const float notch = (kVariableSawNotchDepth + 1.0f - pw_) * notch_amount;
...
void VariableSawOscillator::SetPW(float pw)
{
    ...
    {
        pw_ = fclamp(
            pw, frequency_ * 2.0f, 1.0f - 2.0f * frequency_);
    }
}
...
void VariableSawOscillator::SetWaveshape(float waveshape)
{
  ...
  float VariableSawOscillator::ComputeNaiveSample(
      float phase,
      float pw,
      float slope_up,
      float slope_down,
