                              ...
                              size_t size)
{
    ...
    for(int i = 0; i < kNumModes; ++i)
    {
        f[i] = fmin(f0 * kModeFrequencies[i], 0.499f);
    ...
    while(size--)
    {
        ...
        float sustain_gain_value = sustain_gain_ = accent * decay;
        ...
        for(int i = 0; i < kNumModes; ++i)
        {
            ...
            shell += gain[i] * (sustain ? sin(phase_[i] * TWOPI_F) * sustain_gain_value * 0.25f : resonator_[i].Band() + excitation * exciter_leak);
        }
