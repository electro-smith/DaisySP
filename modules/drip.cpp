...
float Drip::Process(bool trig)
{
    ...
    if (freq1_ != 0.0 && freq1_ != res_freq1_) {
        res_freq1_ = freq1_;
        coeffs10_ = -WUTR_RESON * 2.0 *
        cos(res_freq1_ * tpidsr);
    }
    if (freq2_ != 0.0 && freq2_ != res_freq2_) {
        res_freq2_ = freq2_;
        coeffs20_ = -WUTR_RESON * 2.0 *
        cos(res_freq2_ * tpidsr);
    }
    if ((--kloop_) == 0) {
        shake_energy_ = 0.0;
    }
    ...
    if (my_random(32767) < num_objects) {
        ...
        j = my_random(3);
        if (j == 0) {
            center_freqs0_ = res_freq1_ *
            (0.75 + (0.25 * noise_tick()));
            gains0_ = fabs(noise_tick());
        }
	else if (j == 1)
	{
            center_freqs1_ = res_freq1_ *
            (1.0 + (0.25 * noise_tick()));
            gains1_ = fabs(noise_tick());
        }
	else
	{
            center_freqs2_ = res_freq1_ *
