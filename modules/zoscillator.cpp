...
using namespace daisysp;
  ...
  float ZOscillator::Process(
	...
	formant_frequency_ = formant_frequency;
      ...
      if (reset) {
		...
		float mode_sub = mode_ + (1.f - reset_time) * (mode_ - mode);
		float shape_sub = carrier_shape_ + (1.0f - reset_time) * (carrier_shape_ - carrier_shape);
        float before = Z(
